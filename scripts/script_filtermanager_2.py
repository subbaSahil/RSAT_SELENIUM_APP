...
# Clicking filter manager: SystemDefinedFilterManager
     ...
     for i, div in enumerate(open_divs, start=1):
         ...
         if 'hasOpenPopup' in class_attr:
              filter_manager_cloumn_last_opened = Interactions.get_element_text(driver, By.XPATH, f"(//div/parent::div[contains(@class, 'dyn-headerCell')])[2]")
              break
     if filter_manager_cloumn_last_opened == 'Vendor account' and filter_manager_cloumn_last_opened != '':
         ...
         Interactions.wait_and_click(driver, By.XPATH, "//div[text()='Vendor account']")
     filter_manager_data = Interactions.extract_value_and_operator_from_description("Enter a filter value of '1002' on the 'Vendor account' field using the 'begins with' filter operator.")
...
# Clicking filter manager: SystemDefinedFilterManager
     ...
     for i, div in enumerate(open_divs, start=1):
         class_attr = div.get_attribute('class')
         if 'hasOpenPopup' in class_attr:
              filter_manager_cloumn_last_opened = Interactions.get_element_text(driver, By.XPATH, f"(//div/parent::div[contains(@class, 'dyn-headerCell')])[4]")
              break
     if filter_manager_cloumn_last_opened == 'Vendor account' and filter_manager_cloumn_last_opened != '':
         Interactions.wait_and_click(driver, By.XPATH, "//div[text()='Vendor account']")
         Interactions.wait_and_click(driver, By.XPATH, "//div[text()='Vendor account']")
         ...
         Interactions.wait_and_click(driver, By.XPATH, "//div[text()='Vendor account']")
     filter_manager_data = Interactions.extract_value_and_operator_from_description("Enter a filter value of '100' on the 'Vendor account' field using the 'begins with' filter operator.")
     operator = filter_manager_data['operator']
     new_val = filter_manager_data['value']
     field_name = filter_manager_data['field_name']
     drop_down_item = "//input[contains(@aria-label,'Filter field: "+field_name+",')]/ancestor::div[@class='columnHeader-popup sysPopup']/ancestor::body/child::div[@class='sysPopup flyoutButton-flyOut layout-root-scope']//button//span[text()='"+operator+"']"
